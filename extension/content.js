...
LeetsyncX.prototype.checkSubmissionSuccess = function () {
  const resultTags = document.querySelectorAll(
...
LeetsyncX.prototype.getSolutionStats = function () {
  if (this.solutionDetails) {
    ...
    const runtimePercent =
      this.solutionDetails.runtimePercentile != null
    ...
    const memoryPercent =
      this.solutionDetails.memoryPercentile != null
...
LeetsyncX.prototype.showSuccess = function () {
  ...
  if (spinner) {
    spinner.className = "";
    spinner.style =
      "display: inline-block; transform: rotate(45deg); height: 20px; width: 10px; border-bottom: 5px solid rgb(9, 245, 9); border-right: 5px solid #32cd32;";
  }
};
...
LeetsyncX.prototype.showFailure = function () {
  ...
  if (spinner) {
    spinner.className = "";
    spinner.style =
      "display: inline-block; transform: rotate(45deg); height: 20px; width: 10px; border-bottom: 5px solid rgb(255, 51, 0); border-right: 5px solid #ff4500;";
  }
};

LeetsyncX.prototype.trackSubmissionNavigation = function () {
  window.navigation.addEventListener("navigate", (event) => {
    const problemMatch = window.location.href.match(
    ...
    try {
      ...
      chrome.runtime.sendMessage(syncPayload, (response) => {
        if (response.status === "success") {
          ...
          const difficulty =
            syncHandler.solutionDetails?.question?.difficulty || "Unknown";
          chrome.storage.local.get("difficultyStats", (result) => {
            ...
            chrome.storage.local.get("streakDates", (streakResult) => {
              ...
              if (!streakDates.includes(today)) {
                // Agar aaj ka date nahi hai toh add karo
              ...
              chrome.storage.local.set(
                { difficultyStats: stats, streakDates: streakDates },
                () => {
